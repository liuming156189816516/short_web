<template>
  <div ref="heightEle" class="view_warp">
      ...
      <div class="left_mess">
        <div class="radio_warp">
          <el-radio-group v-model="task_type" size="small" @change="handleTab">
            <el-radio v-for="(item,idx) in labelOption" :key="idx" :label="idx" border>{{ item }}</el-radio>
          </el-radio-group>
        </div>
        <el-row>
          <el-col :span="24">
            ...
            >
              <el-form-item :label="$t('sys_g070')" prop="apply_name">
                <el-input
                  v-model="taskForm.apply_name"
                  :placeholder="$t('sys_mat061',{value:$t('sys_g070')})"
                  clearable
                />
              </el-form-item>
              <template v-if="task_type==0">
                <el-form-item :label="labelOption[task_type]" prop="apply_mobile">
                  <el-input
                    v-model="taskForm.apply_mobile"
                    :placeholder="$t('sys_mat061',{value:$t('sys_s021')})"
                    clearable
                    rows="6"
                    type="textarea"
                  />
                  <p style="margin: 0;color:#99A1B7;line-height: 1;font-size: 14px;margin: 10px 0 5px 0;">
                    手机号以换行符分隔，最多100,000个，更多用导入。如：</p>
              ...
              <template v-if="task_type==1">
                <el-form-item :label="labelOption[task_type]" prop="phone_list">
                  ...
                  <div class="submit_btn">
                    <el-button :disabled="stepsHide" :loading="stepsHide" class="custom_file1" style="margin-top: 0;">
                      {{ stepsHide ? $t('sys_q015') : $t('sys_c059') }}
                      <input id="uploadFile" ref="uploadclear" accept=".txt" type="file" @change="checkDataIsUse">
                    </el-button>
                  ...
                  <transition name="el-zoom-in-top">
                    <div v-if="showFile" class="mobile_list">
              ...
              <template v-if="task_type==2">
                ...
                <el-form-item :label="$t('sys_s027')" prop="apply_total">
                  <el-input
                    v-model="taskForm.apply_total"
                    :placeholder="$t('sys_mat061',{value:$t('sys_s027')})"
                    clearable
              ...
              <el-form-item label="发送方式" prop="send_type">
                <el-select v-model="taskForm.send_type" placeholder="请选择发送方式" clearable>
                  <el-option
              ...
              <template v-if="taskForm.send_type===1">
                <el-form-item label="SenderId">
                  <el-select v-model="taskForm.sender_id" :placeholder="$t('sys_c052')" clearable>
                    <el-option
                      v-for="item in channelkList"
                      v-show="item.status==1"
                      :key="item.id"
                      :label="item.name+' 【'+recomOption[item.channel_type]+'】'"
              ...
              <el-form-item label="发送话术" prop="apply_say">
                <el-row style="margin-bottom: 10px;">
                  <el-button plain type="primary" @click="showShortList">常用短信</el-button>
                </el-row>
                <el-input
                  v-model="taskForm.apply_say"
                  :placeholder="$t('sys_mat061',{value:$t('sys_rai091')})"
                  clearable
                  rows="6"
                  type="textarea"
                  @input="checkTxt"
                />
                <transition name="el-zoom-in-top">
                  <div v-if="sendcode" class="text_check">
                    <div>
              ...
              <el-form-item label-width="0" style="display: flex;justify-content: center;">
                ...
                <el-button :disabled="sendlen!=1" :loading="isLoading" type="primary" @click="submitForm('taskForm')">
                  提交表单
                </el-button>
              </el-form-item>
        ...
        <div class="model_main">
          <div class="mobile_cover">
            ...
            <div :class="['mess_content',taskForm.apply_say?'mess_cover':'']">
              <div v-if="taskForm.apply_say" class="mess_title">
                <p>信息</p>
                <p>今天 {{ viewTime }}</p>
              </div>
              <transition name="run">
    ...
    <el-dialog :visible.sync="shortSource" center title="短信列表" width="850px">
      <el-button class="fr" size="mini" style="margin-bottom: 10px;" type="primary" @click="creatShrtBtn(0,1)">新增短信
      </el-button>
      <el-table
        ref="el_table"
        v-loading="isSloading"
        ...
        style="width: 100%;"
        @cell-click="rowSelectChange"
      >
        <el-table-column align="center" label="#" width="55">
          <template slot-scope="scope">
            <el-checkbox v-model="scope.row.checked" @change="handleAllChange(scope.row)" />
          </template>
        </el-table-column>
        <!-- <el-table-column prop="channel_name" :label="$t('sys_s011')" minWidth="100" />
                <el-table-column prop="channel_id" label="SenderId" width="100" /> -->
        <el-table-column :label="$t('sys_mat019')" min-width="240" prop="content" show-overflow-tooltip />
        <el-table-column :label="$t('sys_l062')" min-width="130" prop="content" show-overflow-tooltip>
          <template slot-scope="scope">
            {{ scope.row.remark || '-' }}
          </template>
        </el-table-column>
        <el-table-column :label="$t('sys_c010')" fixed="right" width="150">
          <template slot-scope="scope">
            <el-button plain size="mini" type="primary" @click.stop="creatShrtBtn(scope.row,2)">{{
              $t('sys_c027')
            }}
            </el-button>
            <el-popconfirm
            ...
            >
              ...
              <el-button slot="reference" plain size="mini" style="margin-left: 10px;" type="danger" @click.stop>
                {{ $t('sys_c028') }}
              </el-button>
            </el-popconfirm>
          </template>
      ...
      <el-form size="small" style="width:100%;margin-top: 20px;">
        <el-form-item style="display: flex;justify-content: center;">
          ...
          <el-button :disabled="!store_row" type="primary" @click="submitBtn">{{ $t('sys_c024') }}</el-button>
        </el-form-item>
      </el-form>
    </el-dialog>
    <el-dialog
      :close-on-click-modal="false"
      :title="this.taskForm.ptype==1?'新增短信':'编辑短信'"
      :visible.sync="creatSource"
      center
      width="560px"
    >
      <el-form
      ...
      >
        ...
        <el-form-item :label="$t('sys_mat019')" prop="content">
          <el-input
            ...
            :placeholder="$t('sys_mat061',{value:$t('sys_s017')})"
            clearable
            rows="4"
            type="textarea"
          />
        ...
        <el-form-item :label="$t('sys_l062')">
          <el-input
            ...
            :placeholder="$t('sys_mat061',{value:$t('sys_l062')})"
            clearable
            rows="2"
            type="textarea"
          />
        ...
        <el-form-item label-width="0" style="display: flex;justify-content: center;">
          ...
          <el-button :loading="showtloading" type="primary" @click="createForm('taskForm')">{{
            $t('sys_c024')
          }}
          </el-button>
        </el-form-item>
    ...
    <el-dialog :close-on-click-modal="false" :visible.sync="showSource" center title="选择素材" width="60%">
      <material
        :key="source_type==1?Math.floor(new Date().getTime()):''"
        :message="childMess"
        @changeEle="getChildren"
        @closeDialog="showSource=false"
      />
    </el-dialog>
    <el-dialog :close-on-click-modal="false" :visible.sync="showLink" center title="添加链接" width="560px">
      <el-form
        ref="linkForm"
        :model="linkForm"
        :rules="linkRules"
      ...
      >
        ...
        <template v-if="source_type==4">
          ...
          <el-form-item prop="card_text" style="margin-bottom: 10px;">
            <el-input v-model="linkForm.card_text" :placeholder="$t('sys_mat102')" :rows="8" type="textarea" />
          </el-form-item>
          <el-form-item>
...
export default {
  ...
  computed: {
    ...
    taskRules() {
      return {
        apply_name: [{
          required: true,
          message: this.$t('sys_mat061', { value: this.$t('sys_g070') }),
          trigger: 'blur'
        }],
        apply_mobile: [{
          required: true,
          message: this.$t('sys_mat061', { value: this.$t('sys_s021') }),
          trigger: 'blur'
        }],
        apply_say: [{
          required: true,
          message: this.$t('sys_mat061', { value: this.$t('sys_rai091') }),
          trigger: 'blur'
        }],
        ...
        apply_total: [{
          required: true,
          message: this.$t('sys_mat061', { value: this.$t('sys_s027') }),
          trigger: 'blur'
        }],
        data_pack_id: [{
          required: true,
          message: this.$t('sys_c089', { value: this.$t('sys_rai090') }),
          trigger: 'change'
        }],
        phone_list: [{
          type: 'array',
          required: true,
          message: this.$t('sys_c089', { value: this.$t('sys_s014') }),
          trigger: 'change'
        }],
        content: [{
          required: true,
          message: this.$t('sys_mat061', { value: this.$t('sys_mat019') }),
          trigger: 'blur'
        }],
  ...
  methods: {
    ...
    rowSelectChange(row) {
      for (let i = 0; i < this.datashortList.length; i++) {
        ...
        if (this.datashortList[i].id == row.id && !row.checked) {
          const item = this.datashortList[i];
          item.checked = true;
          this.store_row = item;
    ...
    async checkDataIsUse() {
      ...
      const { data: { sucess_num, fail_num, repeat_num, country_num, phone_list }} = result;
    ...
    async getShortlist() {
      ...
      this.datashortList = list.map(item => {
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          const params = {
            ...
            send_type: this.taskForm.send_type ? Number(this.taskForm.send_type) : 0,
          }
          if (index != 1) {
    ...
    submitLink(formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          this.showLink = false;
          const index = this.taskForm.materialData[this.is_index];
          if (index) {
            for (let k = 0; k < this.taskForm.materialData.length; k++) {
              if (this.is_index === k) {
                const item = this.taskForm.materialData[k];
                item.content = this.linkForm.card_text;
                this.$set(this.taskForm.materialData, k, item)
              }
            }
          } else {
            const newObj = { id: '', type: this.source_type == 2 ? 5 : 6, }
